...
import { HiCalendar, HiOutlineClock, HiOutlineSun } from 'react-icons/hi';
...
function getGreeting(hour) {
  if (hour >= 4 && hour < 10) return { text: 'Pagi', Icon: FiSunrise };
  if (hour >= 10 && hour < 15) return { text: 'Siang', Icon: HiOutlineSun };
  if (hour >= 15 && hour < 18) return { text: 'Sore', Icon: FiSunset };
  return { text: 'Malam', Icon: FiMoon };
}
...
export default function HeaderDateTimeWidget({ locale = 'id-ID' }) {
  ...
  return (
    <div
      className="flex items-center gap-3 rounded-xl bg-blue-200/20 px-3 py-2 backdrop-blur-md ring-1 ring-blue-200/20 text-slate-900 dark:text-white shadow-sm
                    dark:bg-black/20 dark:ring-white/10"
    >
      {/* greet bubble (mobile-hide) */}
      <span className="hidden sm:flex items-center gap-1.5 rounded-lg bg-blue-200/30 px-2 py-1 text-xs font-medium ring-1 ring-blue-200/40">
        <Icon className="h-4 w-4" />
        <span className="text-slate-900 dark:text-white">Selamat {greet}</span>
      </span>

      {/* date */}
      <span className="inline-flex items-center gap-1.5 text-sm">
        <HiCalendar className="h-4 w-4 opacity-90" />
      ...
      <span className="inline-flex items-center gap-1.5 text-sm tabular-nums">
        <HiOutlineClock className="h-4 w-4 opacity-90" />
        <span className="font-semibold">{timeStr}</span>
        <span className="text-slate-700 dark:text-white/80 text-xs">
          {timezoneLabel}
        </span>
      </span>
    </div>
