...
export default function BugDetail({ bugId, currentUser }) {
  ...
  const fetchBugDetails = async () => {
    try {
      ...
      'Error fetching bug details:', error;
  ...
  const updateBugStatus = async () => {
    ...
    try {
      ...
      toast.success(
        `Status bug berhasil diubah menjadi ${getStatusText(newStatus)}`,
      );
    } catch (error) {
      'Error updating bug status:', error;
    ...
    <div className="container mx-auto p-4">
      ...
      >
        <div className="flex flex-wrap justify-between items-start gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                <Badge color={getStatusBadgeColor(bug.status)}>
                  {getStatusText(bug.status)}
                </Badge>
                <Badge color={getPriorityBadgeColor(bug.priority)}>
          ...
          {currentUser &&
            (currentUser.role === 'ADMIN' || currentUser.role === 'BUPATI') && (
        ...
        <div className="md:col-span-1">
          <Card>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <div
                  className={`p-3 rounded-full ${getStatusBgColor(bug.status)}`}
                >
                  {bug.status === 'RESOLVED' ? (
                    <HiOutlineCheck
                      className={`h-6 w-6 ${getStatusTextColor(bug.status)}`}
                    />
                  ) : (
                    <FaBug
                      className={`h-6 w-6 ${getStatusTextColor(bug.status)}`}
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`p-3 rounded-full ${getPriorityBgColor(bug.priority)}`}
                >
                  <HiOutlineExclamation
                    className={`h-6 w-6 ${getPriorityTextColor(bug.priority)}`}
                  />
                </div>
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="space-y-2">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      <span className="font-medium">Open:</span> Bug baru
                      dilaporkan dan belum ditangani.
                    </p>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      <span className="font-medium">In Progress:</span> Bug
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      <span className="font-medium">Resolved:</span> Bug telah
                      diperbaiki dan ditutup.
                    </p>
                  </div>
                </div>
              </div>
