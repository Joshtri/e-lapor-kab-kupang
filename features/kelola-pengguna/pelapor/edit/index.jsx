...
const updatePelapor = async ({ id, userData }) => {
...
export default function EditPelaporForm({ userId }) {
  ...
  // Update pelapor mutation
  const updatePelaporMutation = useMutation({
    mutationFn: updatePelapor,
    onSuccess: () => {
      toast.success('Akun Pelapor berhasil diperbarui!');
      queryClient.invalidateQueries({ queryKey: ['users', 'PELAPOR'] });
      queryClient.invalidateQueries({ queryKey: ['user', userId] });
      router.push('/adm/users/pelapor');
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || 'Gagal memperbarui pelapor.');
    },
  });
  ...
  const onSubmit = async (data) => {
    ...
    updatePelaporMutation.mutate({ id: userId, userData: submitData });
  };
  ...
  if (isLoading) {
    return <LoadingScreen isLoading={true} message="Memuat data pelapor..." />;
  }

  return (
    <>
      <LoadingScreen
        isLoading={updatePelaporMutation.isPending}
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="mb-6">
          <PageHeader
            ...
            description="Perbarui informasi akun masyarakat (pelapor)"
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <TextInput
                ...
                disabled={updatePelaporMutation.isPending}
              />
            </div>
            ...
            <div className="flex justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <Button
                type="submit"
                color="blue"
                disabled={updatePelaporMutation.isPending || !isDirty}
              >
                {updatePelaporMutation.isPending ? (
                  <>
