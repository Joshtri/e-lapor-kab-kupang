...
export default function CreateStaffOPDForm() {
  ...
  const createStaffMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      reset();
      router.push('/adm/staff-organisasi-perangkat-daerah');
    },
    onError: (error) => {
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="mb-6">
          <PageHeader
            ...
            backHref="/adm/staff-organisasi-perangkat-daerah"
            role="adm"
          />
        </div>
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* OPD Assignment */}
            <div>
              <Label htmlFor="opdId" value="Assign ke OPD" />
              <Select
                id="opdId"
                {...register('opdId')}
                color={errors.opdId ? 'failure' : 'gray'}
                disabled={createStaffMutation.isPending || isLoadingOpd}
              >
                <option value="">Belum di-assign (bisa di-assign nanti)</option>
                {opdList.map((opd) => (
              ...
              {errors.opdId && (
                <p className="text-sm text-red-500 mt-1">{errors.opdId.message}</p>
              )}
            </div>
